function S = F0AM_ModelCore(Met,InitConc,ChemFiles,BkgdConc,ModelOptions,initinfo,SolarParam)
...
if ModelOptions.GoParallel
    ...
    parfor istep = 1:nIc
        [Conc{istep},Time{istep},StepIndex{istep},...
            k_solar{istep},SZA_solar{istep},days_solar{istep},dcaer_ll,initinfo,aer_conc] = ...
            IntegrateStep(istep,nIc,conc_init(istep,:),conc_last,conc_bkgd(istep,:),ModelOptions,...
            Chem,k(istep,:),Sbroad,Sslice(istep,:),Mbroad,Mslice(istep,:),t_start,Cnames);
    end
    ...
    for istep = 1:nIc
        ...
        % delta soa mass output
        dcaer = [dcaer; dcaer_ll];
        % SOA output
        test = [test,sum(aer_conc(:,:),2)];
    end
    
end  % end for/parfor choice

% ==========SOA output table===========
soaOutput = array2table(test','VariableNames', initinfo.oaspc.name);
