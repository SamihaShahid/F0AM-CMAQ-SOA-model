...
%   names               conc(ppb)               HoldMe    
    ...
    'CCHO'            DAQ.CH3CHO(1)                   0;... %acetaldehyde
    'OLE1'              DAQ.C3H6(1)                   0;... %propene
    'BENZ'           DAQ.BENZENE(1)                   0;... %benzene
    'ARO2'             DAQ.FURAN(1)+ DAQ.MEFURAN(1)   0;... %furan; only decay!
    'ISOP'              DAQ.C5H8(1)                   0;... %isoprene
    'ACET'          DAQ.CH3COCH3(1) .*0.5             0;... %acetone
    'RCHO'           DAQ.CH3COCH3(1) .*0.5            0;... %propanal
    'FACD'             DAQ.HCOOH(1)                   0;... %formic acid%
    'AACD'           DAQ.CH3CO2H(1)./0.66             0;... %acetic acid, inlet factor 0.66%
    'BACL'            DAQ.BIACET(1)                   0;... %2,3 butanedione
    'MVK'               DAQ.MVK(1) .*0.5              0;... %MVK
    'MACR'              DAQ.MVK(1).*0.5               0;... %MACR
    'MGLY'            DAQ.MGLYOX(1)                   0;... %methylglyoxal
    'ARO1'          DAQ.FURFURAL(1)                   0;... %2-furfural; only decay!
    'MEK'            DAQ.ACETOL(1)                    0;... %hydroxyacetone
    'CRES'          0.67+0.45+1.22                    0;
    'OLE2'          DAQ.STYRENE(1) + DAQ.MALANHY(1)   0;
    };
...
writetable(data, '/Users/samiha/Desktop/oxy_aro_chem/SIM_1_S07_STD/conc.xlsx');

time_data = num2cell(S.Time);
time_data=cell2table(time_data,"VariableNames",["t"]);
writetable(time_data, '/Users/samiha/Desktop/NMOG_paper/test/t.xlsx');

data = cell2table(S.Chem.Rnames, "VariableNames",["Rxn"]);
writetable(data, '/Users/samiha/Desktop/NMOG_paper/test/rnames.xlsx');

data = num2cell(S.Chem.Rates);
data = cell2table(data);
writetable(data, '/Users/samiha/Desktop/NMOG_paper/test/rates.xlsx');

data = num2cell(S.Chem.f);
data = cell2table(data);
writetable(data, '/Users/samiha/Desktop/NMOG_paper/test/f.xlsx');

data = num2cell(S.Chem.k);
data = cell2table(data);
writetable(data, '/Users/samiha/Desktop/NMOG_paper/test/k.xlsx');

data = num2cell(S.Chem.iG);
data = cell2table(data);
writetable(data, '/Users/samiha/Desktop/NMOG_paper/test/iG.xlsx');
