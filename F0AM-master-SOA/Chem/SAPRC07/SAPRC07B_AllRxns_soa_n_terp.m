...
'CRESRO2';'BENZRO2';'TOLRO2';'XYLRO2';'TRPRXN';'ISOPRXN';'ALKRO2'; 
'LIM';'CAMPH';'LIMRXN';'CAMPHRXN';};
...
i=i+1;
Rnames{i} = ' CRESRO2 + RO2C = RO2C ';
k(:,i) =  3.50e-14;
Gstr{i,1} = 'CRESRO2'; Gstr{i,2} = 'RO2C'; 
fCRESRO2(i)=fCRESRO2(i)-1; fRO2C(i)=fRO2C(i)-1; fRO2C(i)=fRO2C(i)+1;
 %%
...
i=i+1;
Rnames{i} = ' CRES + OH = 0.2*BZO + 0.8*RO2C + 0.8*xHO2 + 0.8*yR6OOH + 0.25*xMGLY +  5.05*XC + 0.8*CRESRO2 + TOLRO2';
k(:,i) =  125.38E-12;
Gstr{i,1} = 'CRES'; Gstr{i,2} = 'OH'; 
fCRES(i)=fCRES(i)-1; fOH(i)=fOH(i)-1; fBZO(i)=fBZO(i)+0.2; fRO2C(i)=fRO2C(i)+0.8; fxHO2(i)=fxHO2(i)+0.8; fyR6OOH(i)=fyR6OOH(i)+0.8; fxMGLY(i)=fxMGLY(i)+0.25; fXC(i)=fXC(i)+5.05; fCRESRO2(i)=fCRESRO2(i)+0.8; fTOLRO2(i)=fTOLRO2(i)+1; 
...
i=i+1;
Rnames{i} = ' ISOP + OH = 0.986*RO2C + 0.093*RO2XC + 0.093*zRNO3 + 0.907*xHO2 + 0.624*xHCHO + 0.23*xMACR + 0.32*xMVK + 0.357*xIPRD + yR6OOH +  -0.167*XC + ISOPRXN';
k(:,i) =  2.54e-11.*exp(410./ T);
Gstr{i,1} = 'ISOP'; Gstr{i,2} = 'OH'; 
fISOP(i)=fISOP(i)-1; fOH(i)=fOH(i)-1; fRO2C(i)=fRO2C(i)+0.986; fRO2XC(i)=fRO2XC(i)+0.093; fzRNO3(i)=fzRNO3(i)+0.093; fxHO2(i)=fxHO2(i)+0.907; fxHCHO(i)=fxHCHO(i)+0.624; fxMACR(i)=fxMACR(i)+0.23; fxMVK(i)=fxMVK(i)+0.32; fxIPRD(i)=fxIPRD(i)+0.357; fyR6OOH(i)=fyR6OOH(i)+1; fXC(i)=fXC(i)+-0.167; fISOPRXN(i)=fISOPRXN(i)+1;
...
i=i+1;
Rnames{i} = ' ARO1 + OH = 0.166*HO2 + 0.482*xHO2 + 0.284*OH + 0.482*RO2C + 0.068*RO2XC + 0.068*zRNO3 + 0.218*xGLY + 0.138*xMGLY + 0.166*CRES + 0.049*xBALD + 0.164*xAFG1 + 0.193*xAFG2 + 0.284*AFG3 + 0.077*xPROD2 +  0.403*yRAOOH + 0.147*yR6OOH + 0.002*XC + TOLRO2';
k(:,i) =  36e-12;
Gstr{i,1} = 'ARO1'; Gstr{i,2} = 'OH'; 
fARO1(i)=fARO1(i)-1; fOH(i)=fOH(i)-1; fHO2(i)=fHO2(i)+0.166; fxHO2(i)=fxHO2(i)+0.482; fOH(i)=fOH(i)+0.284; fRO2C(i)=fRO2C(i)+0.482; fRO2XC(i)=fRO2XC(i)+0.068; fzRNO3(i)=fzRNO3(i)+0.068; fxGLY(i)=fxGLY(i)+0.218; fxMGLY(i)=fxMGLY(i)+0.138; fCRES(i)=fCRES(i)+0.166; fxBALD(i)=fxBALD(i)+0.049; fxAFG1(i)=fxAFG1(i)+0.164; fxAFG2(i)=fxAFG2(i)+0.193; fAFG3(i)=fAFG3(i)+0.284; fxPROD2(i)=fxPROD2(i)+0.077; fyRAOOH(i)=fyRAOOH(i)+0.403; fyR6OOH(i)=fyR6OOH(i)+0.147; fXC(i)=fXC(i)+0.002; fTOLRO2(i)=fTOLRO2(i)+1;

i=i+1;
Rnames{i} = ' ARO2 + OH = 0.108*HO2 + 0.58*xHO2 + 0.202*OH + 0.58*RO2C + 0.11*RO2XC + 0.11*zRNO3 + 0.116*xGLY + 0.286*xMGLY + 0.104*xBACL + 0.108*CRES + 0.039*xBALD + 0.217*xAFG1 + 0.21*xAFG2 + 0.202*AFG3 + 0.08*xAFG3 +  0.035*xPROD2 + 0.089*yR6OOH + 0.601*yRAOOH + 1.486*XC + XYLRO2';
k(:,i) =  46.2e-12;
Gstr{i,1} = 'ARO2'; Gstr{i,2} = 'OH'; 
fARO2(i)=fARO2(i)-1; fOH(i)=fOH(i)-1; fHO2(i)=fHO2(i)+0.108; fxHO2(i)=fxHO2(i)+0.58; fOH(i)=fOH(i)+0.202; fRO2C(i)=fRO2C(i)+0.58; fRO2XC(i)=fRO2XC(i)+0.11; fzRNO3(i)=fzRNO3(i)+0.11; fxGLY(i)=fxGLY(i)+0.116; fxMGLY(i)=fxMGLY(i)+0.286; fxBACL(i)=fxBACL(i)+0.104; fCRES(i)=fCRES(i)+0.108; fxBALD(i)=fxBALD(i)+0.039; fxAFG1(i)=fxAFG1(i)+0.217; fxAFG2(i)=fxAFG2(i)+0.21; fAFG3(i)=fAFG3(i)+0.202; fxAFG3(i)=fxAFG3(i)+0.08; fxPROD2(i)=fxPROD2(i)+0.035; fyR6OOH(i)=fyR6OOH(i)+0.089; fyRAOOH(i)=fyRAOOH(i)+0.601; fXC(i)=fXC(i)+1.486; fXYLRO2(i)=fXYLRO2(i)+1;

i=i+1;
Rnames{i} = ' TERP + OH = 0.759*xHO2 + 0.042*xRCO3 + 1.147*RO2C + 0.2*RO2XC + 0.2*zRNO3 + 0.001*xCO + 0.264*xHCHO + 0.533*xRCHO + 0.036*xACET + 0.005*xMEK + 0.009*xMGLY + 0.014*xBACL + 0.002*xMVK + 0.001*xIPRD +  0.255*xPROD2 + yR6OOH + 5.056*XC + TRPRXN';
k(:,i) =  1.37e-10; % weighted kOH of the rest of the TERP
Gstr{i,1} = 'TERP'; Gstr{i,2} = 'OH'; 
fTERP(i)=fTERP(i)-1; fOH(i)=fOH(i)-1; fxHO2(i)=fxHO2(i)+0.759; fxRCO3(i)=fxRCO3(i)+0.042; fRO2C(i)=fRO2C(i)+1.147; fRO2XC(i)=fRO2XC(i)+0.2; fzRNO3(i)=fzRNO3(i)+0.2; fxCO(i)=fxCO(i)+0.001; fxHCHO(i)=fxHCHO(i)+0.264; fxRCHO(i)=fxRCHO(i)+0.533; fxACET(i)=fxACET(i)+0.036; fxMEK(i)=fxMEK(i)+0.005; fxMGLY(i)=fxMGLY(i)+0.009; fxBACL(i)=fxBACL(i)+0.014; fxMVK(i)=fxMVK(i)+0.002; fxIPRD(i)=fxIPRD(i)+0.001; fxPROD2(i)=fxPROD2(i)+0.255; fyR6OOH(i)=fyR6OOH(i)+1; fXC(i)=fXC(i)+5.056; fTRPRXN(i)=fTRPRXN(i)+1;

i=i+1;
Rnames{i} = ' LIM + OH = LIMRXN';
k(:,i) =  1.61e-10; % kOH of the LIM
Gstr{i,1} = 'LIM'; Gstr{i,2} = 'OH'; 
fLIM(i)=fLIM(i)-1; fOH(i)=fOH(i)-1; fLIMRXN(i)=fLIMRXN(i)+1;

i=i+1;
Rnames{i} = ' CAMPH + OH = CAMPHRXN';
k(:,i) =  5.33e-11; % kOH of the CAMPH
Gstr{i,1} = 'CAMPH'; Gstr{i,2} = 'OH'; 
fCAMPH(i)=fCAMPH(i)-1; fOH(i)=fOH(i)-1; fCAMPHRXN(i)=fCAMPHRXN(i)+1;
